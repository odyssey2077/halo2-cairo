...
impl<F: ScalarField, const CYCLES: usize> CairoVMCircuit<F, CYCLES> {
    ...
    fn read_memory(&mut self, index: AssignedValue<F>) -> AssignedValue<F> {
        ...
        let mut index_usize = index.value().to_repr().as_ref()[0] as usize;
        let mut value = F::ZERO;
        if index_usize >= self.memory.len() {
            index_usize = 0;
        } else {
            value = self.memory[index_usize];
            // save(0, 0) for out of bound access
            index_usize = index_usize + 1;
        }
        println!("index: {}", index_usize);
        let value = ctx.load_witness(value);
        let index = ctx.load_witness(F::from(index_usize as u64));
        self.ram
...
#[test]
fn test_cairo_mock() {
    // has to change it from 5 to 10 otherwise panic with error message
    // thread 'cairo_circuit::test_cairo_mock' panicked at /Users/wheatwaves/.cargo/git/checkouts/halo2-3f7859b18a8cbb9d/e1a04ce/halo2_proofs/src/dev.rs:446:13:
    // row=25, usable_rows=0..25, k=5
    let k = 10u32;
